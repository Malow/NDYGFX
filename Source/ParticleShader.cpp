...
HRESULT ParticleShader::Init(ID3D11Device* device, std::string fileName, const D3D11_INPUT_ELEMENT_DESC* inputElementDesc, unsigned int nrOfElements)
{
	HRESULT hr = S_OK;

	this->mDevice = device;

	this->m_pFileName = new std::string(fileName);
...
const std::string* ParticleShader::GetFileName() const
{
