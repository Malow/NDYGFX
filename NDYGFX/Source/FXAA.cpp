...
void FXAA::PreRender(Shader* shader, unsigned int preset)
{
	...
	//release
	if ( backBufferTex ) backBufferTex->Release(), backBufferTex = 0;
	if ( sceneTex ) sceneTex->Release(), sceneTex = 0;
	if ( backBufferRTV ) backBufferRTV->Release(), backBufferRTV = 0;
	if ( backBufferRTVResource ) backBufferRTVResource->Release(), backBufferRTVResource = 0;
	if ( sceneSRV ) sceneSRV->Release(), sceneSRV = 0;
}
