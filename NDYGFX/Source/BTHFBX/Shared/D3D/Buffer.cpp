	...
	case VERTEX_BUFFER:
		{
			...
			UINT32 offset = 0;
			mDevice3D->GetDeviceContext()->IASetVertexBuffers(misc, 1, &mBuffer, &vertexSize, &offset );
		}
		break;
	case INDEX_BUFFER:
		{
			mDevice3D->GetDeviceContext()->IASetIndexBuffer(mBuffer, DXGI_FORMAT_R32_UINT, 0);
		}
		break;
	case CONSTANT_BUFFER_VS:
		{
			mDevice3D->GetDeviceContext()->VSSetConstantBuffers(misc, 1, &mBuffer);
		}
		break;
	case CONSTANT_BUFFER_GS:
		{
			mDevice3D->GetDeviceContext()->GSSetConstantBuffers(misc, 1, &mBuffer);
		}
		break;
	case CONSTANT_BUFFER_PS:
		{
			mDevice3D->GetDeviceContext()->PSSetConstantBuffers(misc, 1, &mBuffer);
		}
		break;
	default:
		hr = E_FAIL;
...
HRESULT Buffer::Init(Device3D* device3D, BUFFER_INIT_DESC& initDesc)
{
	mDevice3D = device3D;
	...
	if(initDesc.InitData)
	{
		...
		hr = mDevice3D->GetDevice()->CreateBuffer(&bufferDesc, &data, &mBuffer);
	}
	else
	{
		hr = mDevice3D->GetDevice()->CreateBuffer(&bufferDesc, NULL, &mBuffer);
	}
...
void* Buffer::Map()
{
	...
	if(mUsage == BUFFER_CPU_WRITE || mUsage == BUFFER_CPU_READ || mUsage == BUFFER_CPU_WRITE_DISCARD)
	{
		...
		HRESULT hr = S_OK;
		if(FAILED(hr = mDevice3D->GetDeviceContext()->Map(
			mBuffer,
			0,
			(D3D11_MAP)mapType,
			0,
...
void Buffer::Unmap()
{
	mDevice3D->GetDeviceContext()->Unmap( mBuffer, 0 );
}
